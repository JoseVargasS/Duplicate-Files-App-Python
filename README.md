...
App local para revisar fotos, videos y documentos duplicados. Tambien puede encontrar imagenes visualmente parecidas, capturas o imagenes con texto, y miniaturas de baja calidad. Todo corre en tu maquina y los archivos descartados se mueven a una carpeta de respaldo.
...
- En revisiones de texto/miniaturas, casilla `Mover` en cada tarjeta para decidir candidato por candidato.
- Revision opcional de capturas o imagenes con texto.
...
Instala las dependencias Python para miniaturas, dimensiones, PDFs y OCR opcional:
...
Para mejor deteccion de capturas con texto, instala tambien Tesseract OCR en Windows. La app lo busca automaticamente en `C:\Program Files\Tesseract-OCR\tesseract.exe`; si lo instalas en otra ruta, define la variable `TESSERACT_CMD`.

Si usas los scripts npm, `npm run setup` instala `pillow`, `pymupdf` y `pytesseract` en el Python que usa la app.
...
image_tools.py          Escaneo, hash, calidad, texto/capturas, videos y documentos
...
4. Activa `Capturas o imagenes con texto` para revisar screenshots, memes o fotos con texto visible.
...
- `Capturas o imagenes con texto`: usa nombres comunes, una heuristica liviana por zonas de texto y OCR opcional con `pytesseract` solo en candidatos probables.
